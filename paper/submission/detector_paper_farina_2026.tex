...
$p = 0.003$ out of sample; every shorter buffer is indistinguishable
from zero, and one returns the wrong sign), and
...
release an automated checklist that recovers six of the eight defects
from source, and record that its authors committed the same class of
error seven times while writing this paper --- including an instrument
validated by eye from an insignificant correlation, in the section
arguing that unvalidated instruments are worthless, and a check that,
like the detector it was built to replace, could not fail.
...
\item \texttt{detector\_audit.py}, an eight-check runner that recovers
      six of the eight defects from source and archived data, with the
      limits of the other two stated.
\item A record of seven errors of the same class committed by the
...
Re-running 100 seeds \emph{in 1D} at 1-tick sampling, the gated delay
collapses to exactly 1 tick in 91 of the 97 runs in which both
predicates latched (93.8\,\%). Three of the 100 seeds (15, 39, 44)
never latched the Map predicate and are excluded from that denominator;
the six remaining runs did latch both, at delays of 112, 134, 232, 318,
383 and 634 ticks, which is what lifts the gated mean to 19.6. The
2D 1-tick re-run covers only four seeds and is too small to quote. The
delay is not a property of the system. It is the measurement grid.
...
Map predicate crossed before CV was computable in 476 of 482 archived 1D
runs (98.8\,\%) and 198 of 198 2D runs. The six 1D exceptions are seeds
5, 171, 202, 219, 294 and 361. (A draft reported 115 of 116 and 118 of
118 here: the same 120-run subset disavowed two paragraphs above,
carried into the sentence that follows the disavowal.)
...
Clock time for this configuration ($\sim$4{,}625 ticks, the median
unplanted Clock crossing over the 38 runs in which both predicates
fired; the archive median of 4{,}200 in Defect 3 is the same quantity
measured at a different run length and seed count).}
\begin{tabular}{rrrr}
\toprule
...
the 200 runs in which both predicates fired and quoted here only as a
bound, since at this effect size the exact value is an artifact of the
sample size rather than evidence about the ordering);
...
natural division period ($\sim$2{,}540 ticks) forced division splits
cells before they have grown, reaching 79.4\,\% premature splits at
$T = 1600$ and 100\,\% at $T = 200$. (A draft gave $\sim$3{,}170 here
and explained the discrepancy with the $\sim$2{,}500 quoted elsewhere
as a difference of configuration. That explanation was wrong. The 3{,}170
came from an analysis script that divides by a hard-coded 50{,}000-tick
run length for ablation runs that were in fact executed at 40{,}000
ticks; recomputed at the true length the same archive gives
$\sim$2{,}540, in agreement with every other period in this paper. We
had reconciled two numbers by inventing a distinction rather than
checking either.) Within the valid window:
...
is the third-\emph{highest} of the nine, not the lowest --- behind
$\mathrm{CV} = 1.0$ (0.781) and $\mathrm{CV} = 0.8$ (0.773) --- so a rank
correlation computed across the whole range averages a decrease and an
increase and reports the net. We flag this rather than resolve it, and we withdraw the explanation we
first reached for. A draft of this paragraph attributed the
$\mathrm{CV} = 0$ point to survivorship, on the ground that
$\mathrm{CV} = 0$ is also the condition with 27.3\,\% extinction
(\S\ref{sec:survives}). That is wrong. The 27.3\,\% is a marginal
over all six imposed periods and is produced entirely by
$T \leq 800$ --- periods this table excludes as unphysical. Within the
table's own window ($T \in \{3200, 6400\}$) there are \emph{zero}
extinctions at every $\mathrm{CV}$ level, 0 of 50 at $\mathrm{CV} = 0$,
and all 450 runs contribute a finite steady-state $S$. The point is not
survivorship-filtered and we have no mechanism for it. What the data
support is narrower: imposed regularity does not raise stability
anywhere in this grid, and the single number $+0.2196$ should not be
read as a dose--response. This error is recorded as entry (7) of
\S\ref{sec:auditors}.
...
Two criteria were pre-registered. $\rho(\text{min interval}, S) > +0.25$
passes on the pooled data ($+0.3844$, $n = 153{,}133$
cell-observations, natural division). The second --- a gap of at least
0.15 in mean $S$ between the highest and lowest minimum-interval
quintiles --- fails on that same pooled data before any stratification
is applied, at $+0.1229$ (0.7825 to 0.9054). A draft mentioned only the
first, which made the reformulation look as though stratification alone
defeated it. Stratified by division count, the first criterion fails
too:
...
pre-registered disqualifier fires. The secondary arm (forced division)
gives $\rho = -0.1484$ pooled and does not reverse under
stratification, but we withdraw the way a draft summarised it. That
draft reported it as ``strengthening to $-0.3463$'', which is one
stratum (5--8 intervals, $n = 5{,}305$) holding 3.8\,\% of the arm; the
two strata carrying 96.2\,\% do not strengthen ($-0.1871$ at
$n = 102{,}125$ and $-0.1424$ at $n = 32{,}268$, the latter weaker than
pooled). Quoting the most favourable stratum as the arm's result is the
mirror of the pooling error this section is about. Stated properly:
neither arm supports longer minimum intervals producing more stable
patterns, and the secondary arm's association is stable but weak
throughout.
...
$p = 0.241$ over 40 seeds. The division period in the configuration
that arm ran in is $\sim$2{,}625 ticks and the lag window could not
reach it.

Widening the buffer to 6{,}000 ticks --- 2.4 division periods --- does
not fix it. In the 40{,}000-tick buffer sweep below, that arm gives
$\rho = +0.2162$, $p = 0.186$: the wrong sign, and insignificant. An
earlier draft reported $\rho = -0.2364$ for the same buffer and treated
it as validation. That figure is the first ten seeds of a separate
20{,}000-tick run, it was never significant ($p = 0.511$), and at the
full 40 seeds of that run it is $-0.0822$ ($p = 0.614$). Neither run
supports the arm; we distinguish them here because the draft quoted
both as though they were one experiment. Quoting a correlation
...
\begin{table}[H]
\centering
\caption{Buffer sweep, 40 seeds per arm at 40{,}000 ticks; one seed
went extinct in every arm, so each correlation is over $n = 39$.
Validity requires $\rho(\mathrm{clock}_r, \mathrm{CV}) < 0$ and
significant. The definedness column is the median.}
...
Two features carry the argument. \textbf{Definedness tracks buffer
length}, which is near enough an identity to be treated as one:
$\mathrm{clock}_r$ exists the moment its buffer fills. It is exact in
194 of the 200 runs across all arms; the six exceptions are all
\emph{upward} (6{,}850, 12{,}850, 24{,}850, 30{,}350, 33{,}600, and one
run never defined), where a lineage died and its replacement had to
refill from empty. The identity is therefore a lower bound, which is the
direction that matters here: whatever span validity demands is at best
the earliest the metric can exist. And \textbf{the approach is not
monotone} --- the 2.36-period arm has the wrong sign and the 4.71-period
arm is flat --- so we claim no smooth relationship between span and
validity, only that the intermediate arms are uninformative at
$n = 39$.
...
$\rho = -0.4565$, $p = 0.0031$, with $\mathrm{clock}_r$ defined in all
40 runs --- at exactly tick 24{,}000 in 39 of them, and at 30{,}350 in
the fortieth. That is a single pre-specified test of a
...
During the audit we published seven claims to the public repository
that we subsequently withdrew. They fall into four classes. Six were
found by an adversarial numeric fact-check of the draft; the seventh was
introduced by the revision that fixed them.
...
\textbf{Explanation invented for an unexplained number} (one
occurrence), committed in the revision that fixed the six above.
\begin{enumerate}\setcounter{enumi}{6}
\item \S\ref{sec:rescue} reports a non-monotone response of $S$ to
      imposed regularity. Having criticised the original paper for
      explaining an artifact with physics rather than checking
      \texttt{len()}, we explained the awkward $\mathrm{CV} = 0$ point
      by survivorship, citing the 27.3\,\% extinction at that
      condition. The table excludes every period at which those
      extinctions occur; within its own window the extinction rate is
      zero at all nine $\mathrm{CV}$ levels. One query against the grid
      would have shown it. We wrote the sentence instead, in the
      paragraph whose purpose was to disclose that a summary statistic
      had concealed the shape of the data.
\end{enumerate}

An eighth case deserves separate mention, being a near-miss rather than
...
demonstrated seven times above, by people actively looking for it, in a
...
\item \textbf{C3} any reported delay whose median equals the
      quantisation grid inferred from the delays themselves, with a
      majority sitting exactly on it
\item \textbf{C4} any summary reporting a mean beside an
      $|\mathrm{SD}| > |\mathrm{mean}|$ (critical with no median
      present, major with one)
...
Run against this repository the tool reports 31 findings, 16 critical,
and recovers \emph{six} of the eight defects --- 1, 2, 3, 4, 6 and 7 ---
from source and archived data. It independently locates the $n = 2$
comparison group that took three rounds of expert review and four months
to notice by eye, and it flags precisely the two results the authors
wrongly retained.

A draft claimed all eight, which was not true and is worth stating
plainly. Defect 5, the denominator, is not recovered: no check reads
\texttt{results/ablations/}, because both data checks glob
\texttt{summary.csv} and the file is named
\texttt{ablation\_summary.csv}. Defect 8 is not recovered either. C8
detects that \emph{class} --- it finds a genuine Simpson's paradox in
$\rho(\text{min interval}, S)$ --- but never recomputes Defect 8's own
quantity, $\rho(T_{\mathrm{div}}, S)$ over the lipid ablations, because
its source globs are hard-coded elsewhere. Finding the same class of
error in a different place is not the same as recovering the error, and
we had counted it as such.

The ``no audit knowledge encoded'' claim needs narrowing too. C6
hard-codes the column \texttt{final\_mean\_s} and the thresholds
0.1/0.3; C8 hard-codes its column names and stratum bins; and C7 has no
detection logic of its own --- it reads a hand-written manifest whose
entries already say \texttt{(DETECTOR OUTPUT)}. C1--C5 read source and
data directly and are the checks that generalise.

Three checks are regular-expression based and tuned to this codebase's
idioms; the tool is a floor, not a ceiling.

\subsection{The tool went blind, and said nothing}

A draft of this section reported 30 findings. Re-running the tool for
this revision returned 29, and the discrepancy is worth the space.

Between the two runs we had attached a staleness disclosure to
\texttt{web/results/summary.csv} --- the archived copy served by the
public dashboard --- as a block of \texttt{\#} comment lines, having
decided against silently repointing a file on a live service.
\texttt{csv.DictReader} took the first comment line as the header row.
Every expected column was therefore absent, and each check guarded
itself with a clause of the form \texttt{if not rows or KEY not in
rows[0]: continue}. The file was skipped. Two genuine findings on it ---
a C3 delay-equals-interval and a C6 degenerate split --- disappeared
from the report. Nothing raised, nothing logged, no error.

The failure is our own thesis in miniature, committed in the instrument
built to detect it. A checker that silently skips what it cannot parse
reports a clean bill of health for files it never opened, and the number
that would have revealed it --- the total --- moved by one, in a
direction easily read as improvement. It went unnoticed for a day
because the audit was not re-run after the disclosure was added.

The remedy is now in the tool. CSV reading strips comment lines, and
every call site declares the columns it requires; a file that cannot be
parsed, or that lacks them, produces a \emph{finding} rather than a
skip. We recommend the general form of this for any automated check:
\textbf{absence of evidence must be logged as loudly as evidence.}
Restoring the two findings, and with C8's contribution, the corrected
total is 31.

\subsection{And C3 could not fail}

The same fact-check found a worse one. As shipped, C3 computed its
reference grid as \texttt{si = sample\_interval or int(med)}, and
\texttt{sample\_interval} was \texttt{None} at every call site --- so
the ``sampling interval'' was \emph{defined to be the median} it was
about to be compared against. Its second clause,
\texttt{(a <= med).mean() > 0.5}, is true of almost any sample by the
definition of a median. The check fired on any delay column with an
integral median, and could not return a pass.

This paper's Defect 1 is a predicate that cannot emit a violation. We
reproduced it, in the tool written to detect it, and did not notice
because the check kept returning the answer we expected on the data we
already knew was defective. A check is only evidence if it can come back
negative, and we never gave it the chance.

C3 now infers the quantisation grid from the delays themselves (their
GCD) and requires the median to equal one grid step \emph{and} a
majority to sit exactly on it --- a condition real data can fail. It
still flags all three archived summaries at 50 ticks (90.0\,\%,
97.5\,\%, 90.0\,\% exactly on grid). We have added
\texttt{tools/test\_detector\_audit.py}, which supplies every check
with input on which it is required to stay \emph{silent}. That file is
the part of this section we would most like to see copied: an audit tool
without negative controls is an instrument nobody has calibrated.
...
\item \textbf{Extinction under forced fast division.} 32.4\,\%
      population extinction ($73/225$) at the shortest imposed period,
      8.4\,\% at the next, zero at periods $\geq 1600$. The percentage
      is a marginal of the $9 \times 6$ grid, averaging nine
      $\mathrm{CV}$ levels that range from $25/25$ to $2/25$.
\item \textbf{Extinction under enforced synchrony.} 27.3\,\%
      extinction ($41/150$) at imposed $\mathrm{CV} = 0$ versus
      1.3--4.0\,\% at $\mathrm{CV} \geq 0.1$. Lockstep division depletes
      shared resource simultaneously; heterogeneity leaves some lineages
      off-phase when the trough arrives. This is counterintuitive and,
      to our knowledge, untested experimentally.

      \emph{Both results are confined to a regime this paper calls
      unphysical.} All 94 extinctions in the 1{,}350-run grid occur at
      $T \leq 800$ --- the periods \S\ref{sec:rescue} excludes because
      forced division there splits cells before they have grown. At
      $T \geq 1600$ the extinction rate is zero at every
      $\mathrm{CV}$ level, including $\mathrm{CV} = 0$. So the
      synchrony effect is real but is a statement about populations
      already being driven faster than they can grow, not about
      synchrony at viable division rates. We keep both as survivors
      because the arithmetic holds and the effect is a genuine feature
      of the model under stress; we do not claim either bears on
      protocells dividing at sustainable rates. Reported without this
      scope, the synchrony result would be the most misleading sentence
      in the paper.
\end{itemize}
...
that recovers six of them runs in under a second. Most of the four
