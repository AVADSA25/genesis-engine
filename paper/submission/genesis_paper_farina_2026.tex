...
The origin of biological agency --- the point at which physical matter begins to act to preserve its own organization --- remains a central unsolved problem in abiogenesis research \citep{pross2012whatislife,deamer2019assembling,pascal2013towards}. Three major frameworks have dominated the field for decades: \emph{replication-first}, emphasizing RNA world chemistry and template copying \citep{joyce2002antiquity,robertson2012origins,orgel2004prebiotic}; \emph{metabolism-first}, emphasizing autocatalytic networks and free-energy extraction \citep{wachtershauser1988before,kauffman1986autocatalytic,shapiro2006small,russell1997emergence}; and \emph{compartmentalization-first}, emphasizing lipid vesicle self-assembly \citep{deamer1997first,szostak2001synthesizing,luisi2006emergence}. Each framework has accumulated compelling experimental and computational support for its favored mechanism. Each has also struggled to explain how its favored mechanism couples to the others to produce a coherent, evolving agent.
...
\paragraph{2D Sphere Study.} $N = 200$ runs, 10{,}000 ticks each, 16 workers. Each cell ran Gray-Scott on a 642-vertex icosphere with 90 RD substeps per tick and forward-Euler integration. Total runtime: 4 hours 14 minutes.
...
Primary hypothesis test: binomial test that \texttt{clock\_before\_map} $=$ True in all runs where both B and C were reached, against a null hypothesis of random ordering ($p = 0.5$). Secondary test: one-sided Wilcoxon signed-rank test on the distribution of $(C_\mathrm{tick} - B_\mathrm{tick})$ differences, with alternative hypothesis that the median delay is strictly greater than zero.
...
\midrule
Lipid supply         & 97/97 (100\%) & 97/97 (100\%) & 98/98 (100\%) \\
RD noise ($\sigma$)  & 97/97 (100\%) & 97/97 (100\%) & 97/97 (100\%) \\
Growth perturbation  & 97/97 (100\%) & 97/97 (100\%) & 97/97 (100\%) \\
Stability window     & 97/97 (100\%) & 97/97 (100\%) & 97/97 (100\%) \\
\bottomrule
\end{tabular}
\caption{\label{tab:ablation}Parameter sensitivity ablation. Each cell shows (runs with Clock before Map) / (runs where both phases reached). Total across all 12 conditions: 1{,}165 / 1{,}165 = 100.0\%. The ordering principle is robust across parameter variations spanning more than an order of magnitude in several dimensions. Parameter names correspond to code identifiers: Lipid supply = \texttt{LIPID\_SUPPLY}, RD noise = \texttt{RD\_NOISE}, Growth perturbation = \texttt{GROWTH\_PERTURB}, Stability window = \texttt{STAB\_WINDOW}. Full parameter definitions are in the code repository.}
\end{table}
...
The 2D mean delay of 56 ticks approaches our 50-tick sampling floor --- the minimum resolvable $C - B$ gap given that phase predicates are evaluated every 50 ticks --- which Figure~\ref{fig:comparison} renders explicitly. That the delay nonetheless remains strictly positive across all 198 runs, rather than occasionally producing negative values due to measurement granularity, preserves the ordering evidence on the sphere even at the resolution limit.
...
The mechanism underlying our observed ordering is straightforward once articulated. Persistent spatial organization --- whether implemented as classical Turing patterns \citep{turing1952chemical,murray2003mathematical}, phase-separated biomolecular condensates \citep{brangwynne2009germline,hyman2014liquidliquid}, lipid microdomains and rafts \citep{simons1997functional,lingwood2010lipid}, or surface-mediated catalytic networks \citep{wachtershauser1988before,russell2014drive} --- requires uninterrupted time to develop. The reaction-diffusion field, the chemical gradient, the spatial concentration profile: all of these structures accumulate through repeated application of the underlying dynamics, and all are disrupted by division events.
...
\paragraph{The ``replication first vs.\ metabolism first'' debate} \citep{joyce2002antiquity,shapiro2006small,pross2012whatislife} is reframed by our finding. Neither mechanism comes first in the sense of being the primary substrate of life. Both presuppose temporal regularity as a precondition --- replication requires reliable generational timing, and metabolic efficiency requires reliable spatial organization to avoid the ``metabolic bottleneck'' documented in \citet{farina2025engine}. The correct primary question becomes: what produces temporal regularity? Our answer --- membrane mechanics via the Adder mechanism \citep{chen2004kinetic,zhu2009coupled,jun2015cellsize} --- is independent of both replication and metabolism, and consistent with \citeauthor{england2013statistical}'s \citeyearpar{england2013statistical} proposal that thermodynamic driving forces reliably sculpt matter toward organized dissipative states. On our reading, the Adder mechanism is a specific instance of that general principle: membrane mechanics coupled to lipid supply produces temporally regularized dissipation before any information-theoretic or metabolic machinery is required.

\paragraph{Connection to Assembly Theory.} Our mandatory-assembly claim has natural affinity with recent work framing life and selection as products of ordered assembly pathways \citep{sharma2023assembly,marshall2021identifying}. Assembly theory quantifies the ``assembly index'' of a molecular structure --- the minimum number of recursive joining operations required to construct it --- and proposes that high assembly index combined with high abundance is a signature of selection. The Trinity ordering articulates an analogous but distinct claim at the systems level: the assembly of an autonomous dissipative agent requires a specific sequence of coupled mechanisms, and skipping steps forecloses the process. Whether the two frameworks can be unified --- whether the Clock $\to$ Map $\to$ Engine sequence corresponds to a minimum assembly path in the assembly-theoretic sense --- is a promising direction for future theoretical work.

\paragraph{Empirical test of Map\,$\to$\,Clock back-reaction (dimensional asymmetry).}\footnote{Motivated by an external review by Google's Gemini Deep Think (May 2026), which asked whether Map latching could destabilize the Clock through shared membrane-mechanical substrate. The supplement repository contains the full re-analysis pipeline (\texttt{analysis\_v51.py}, \texttt{analysis\_v51b.py}) and per-run data.} Our model treats the Clock as driven by geometric instability and the Map as acting only on the Engine; a sceptic could ask whether, in practice, the spatial dynamics nevertheless feed back into Clock regularity. Post-hoc re-analysis of the existing data tests this directly. For each run that reached Phase C we computed the change in population-mean division CV between a stabilised early-post-latch window (phase\,C\,$+\,2{,}000$–$3{,}000$ ticks in 1D, $+\,500$–$1{,}500$ in 2D) and a long-run late window (phase\,C\,$+\,50{,}000$–$60{,}000$ ticks in 1D, $+\,3{,}500$–$4{,}500$ in 2D). On the 1D ring the Clock continues to tighten indefinitely after Map latching ($n = 480$, mean $\Delta\mathrm{CV} = -0.215$, all 480 runs negative, two-sided sign-test $p = 6.4 \times 10^{-145}$). On the 2D icosphere we observe the opposite trend: a small but statistically significant Clock destabilization ($n = 157$, mean $\Delta\mathrm{CV} = +0.057$, 93 up vs 64 down, sign-test $p = 0.025$). To rule out a generic ``later-in-run population-age drift'' artifact, we ran a permutation null in which each run's $\mathrm{phc}$ value was replaced by another run's $\mathrm{phc}$ before re-evaluating $\Delta\mathrm{CV}$. Across $N = 2{,}000$ shuffles, the null distribution of mean $\Delta\mathrm{CV}$ centered at $-0.107$ with sd $0.013$, and \emph{zero} shuffles produced a value as positive as the observed $+0.057$ (permutation $p < 1/2000$). A complementary fixed-tick control (windows anchored at the median $\mathrm{phc}$ for every run rather than at each run's own $\mathrm{phc}$) yielded $\Delta\mathrm{CV} = -0.007 \approx 0$. Both controls confirm the 2D effect is genuinely anchored to each run's Phase-C latch event, not to absolute simulation time. The 2D effect is roughly 4\% of the 0.25 latch threshold --- far from collapse, but non-zero and Phase-C-specific. We interpret this dimensional asymmetry as consistent with the geometric prediction that richer 2D spatial topology offers more coupling channels back into membrane mechanics. The published model's independent-substrate assumption is therefore conservative on a topologically trivial substrate and slightly but robustly violated in 2D --- a quantitative anchor against which a fully coupled Map$\leftrightarrow$Clock v6 model can be compared. We flag this as a partial empirical confirmation, not a refutation, of the sequential-assembly principle: the Clock survives Map latching across all 637 runs analysed; it merely tightens less robustly in higher dimension.

\paragraph{Empirical test of the Damk\"ohler ``Clock speed-limit'' constraint.}\footnote{Motivated by a second external review by Gemini Deep Think (May 2026), which asked whether the framework implicitly requires a critical Damk\"ohler ratio $\mathrm{Da} = T_{\mathrm{div}} / \tau_{\mathrm{pattern}} > 1$: even a perfectly regular Clock would prevent Map formation if its absolute period $T_{\mathrm{div}}$ were shorter than the pattern-consolidation time $\tau_{\mathrm{pattern}}$. The supplement (\texttt{analysis\_v52\_damkohler.py}, \texttt{damkohler\_speed\_limit.md}) contains the full pipeline.} Our claim that division regularity is sufficient for pattern consolidation must in principle be qualified by a separation-of-timescales requirement: if the cell-cycle period $T_{\mathrm{div}}$ becomes shorter than the time $\tau_{\mathrm{pattern}}$ required for spatial chemistry to lock in, the Map cannot form regardless of Clock regularity. The framework therefore implicitly assumes $\mathrm{Da} = T_{\mathrm{div}} / \tau_{\mathrm{pattern}} > 1$. Our existing 3-level \texttt{LIPID\_SUPPLY} ablation (0.008 / 0.015 / 0.025, $n = 100$ runs $\times$ 50{,}000 ticks each) provides a direct empirical handle: higher lipid supply produces faster division (shorter $T_{\mathrm{div}}$), without changing the spatial chemistry. We estimate $T_{\mathrm{div}}$ as $N \cdot T_{\mathrm{run}} / D_{\mathrm{total}}$ per run and $\tau_{\mathrm{pattern}}$ as the mean Phase\,B-to-Phase\,C interval. Tripling the lipid supply shortens $T_{\mathrm{div}}$ from $5{,}101$ to $2{,}102$ ticks and drops $\mathrm{Da}$ from $99.6$ to $31.5$, yet the headline ordering (Clock\,$\to$\,Map\,$\to$\,Engine) holds in all three conditions (97–98\% Phase-C success). Critically, however, every secondary quality metric degrades exactly as Gemini's hypothesis predicts: mean final pattern stability falls from $0.823$ to $0.644$ (Mann--Whitney $U$, one-sided low $>$ high: $p = 1.34 \times 10^{-24}$); Phase-D (Engine) success drops from 90\% to 79\% (bootstrap $\Delta = +11$\,pp, 95\% CI $[+1, +21]$); and the pooled per-run rank correlation between $T_{\mathrm{div}}$ and final $S$ is $\rho = +0.43$ ($p = 8.9 \times 10^{-15}$). All three signals run in the direction Gemini predicted, even though Clock regularity (CV) \emph{improves} with lipid supply over the same range ($0.075 \to 0.046$). We interpret this as partial empirical confirmation of the speed-limit constraint: the system tested here lives at $\mathrm{Da} \gg 1$, so the Trinity ordering is unbroken, but the framework's robustness pays a measurable quality cost as $\mathrm{Da}$ decreases. The critical $\mathrm{Da} \to 1$ regime remains untested and is a clear target for a v6 ablation deliberately stressing lipid supply to extreme values. This also yields a second falsification criterion to complement Prediction 1: a population with division CV well below 0.15 but with $T_{\mathrm{div}} \ll \tau_{\mathrm{pattern}}$ should fail to reach Phase C --- a ``too fast to organize'' regime distinct from the existing ``too irregular to organize'' regime.

\paragraph{Empirical test of a hypothesized ``Phase E'' selection layer.}\footnote{Motivated by a third external review by Gemini Deep Think (May 2026), which argued that the v5.1c 2D back-reaction implies an unstated ``Phase E: The Map Controls the Clock'' --- an Engine-level filter against Maps that suicide the Clock. Pipeline: \texttt{analysis\_v53\_phaseE.py}, \texttt{phaseE\_selection\_test.md}.} A natural extension of the back-reaction finding is the prediction that, to avoid evolutionary suicide, the thermodynamic Engine must selectively favor Maps that do not destabilize the Clock that birthed them. Real biology supplies an existence proof: the bacterial Min system actively positions FtsZ at the division equator, converting the spatial Map into an active assistant of division rather than its disruptor \citep{lutkenhaus2007min}. We tested this directly on the existing 2D data. For each of the 198 runs that reached Phase C, we split by Phase-D outcome and compared their back-reaction $\Delta\mathrm{CV}$: runs that reached Phase D (Engine, $G_D$, $n = 96$) showed mean $\Delta\mathrm{CV} = +0.101$, while runs that stalled at Phase C ($G_C$, $n = 84$) showed mean $\Delta\mathrm{CV} = -0.024$. The bootstrap 95\% CI on the difference is $[+0.106, +0.144]$, excluding zero by a wide margin --- in the \emph{opposite} direction from the Phase-E selection hypothesis. Engine-reaching runs have \emph{more} back-reaction than stalled runs, not less. Mechanistically this is straightforward: Phase D requires high pattern stability $S$, a strong Map in 2D necessarily produces a larger back-reaction, and the basic thermodynamic Engine in our model does not discriminate between Maps that destabilize the Clock and those that do not. We interpret this null result as scientifically more interesting than confirmation would have been: \textbf{Phase-E selection is not an auto-emergent property of coupled dissipative systems --- it must be evolved.} The Min system Gemini cited is a highly evolved protein-level selection layer, not a primordial physical property, and the gap between our model and Min-equipped bacteria is precisely the gap natural selection had to fill. This refines the framework's claims: the Trinity (Clock, Map, Engine) is what physics provides for free; the additional ``Map controls the Clock'' selection layer is what biology had to engineer to make the Trinity sustainable across generations. A v6 model with an explicit fitness penalty on $\Delta\mathrm{CV}$ should be able to quantify how strong an evolutionary pressure is needed, and over how many generations, to convert the primordial $\Delta\mathrm{CV} \approx +0.10$ regime into a Min-equipped $\Delta\mathrm{CV} \approx 0$ regime.

\paragraph{The nature of the origin-of-life probability problem} may shift. If agency emerges through a specific mandatory sequence rather than through any of many possible paths, and if the first step (membrane-driven division regularity) arises from well-characterized physical chemistry requiring only fatty acids and an energy source, the transition from non-life to life may become more tractable to model than the astronomical-improbability framings suggest \citep{hoyle1981evolution,monod1971chance}. We emphasize that our simulations demonstrate the ordering within a specific coupled-dissipative-system class; the broader claim that prebiotic chemistry inevitably traverses this sequence is a conjecture our results motivate rather than establish. The productive question our framework raises is: given a prebiotic environment with lipids and thermodynamic gradients, under what conditions does the Clock emerge, and how robustly does the Map follow?

\paragraph{The interpretation of fossil and synthetic-biology evidence} gains structure. Transitional systems should exhibit Clock without Map (Phase B but not C) or Clock with incomplete Map (Phase C but not D), but never Map without Clock. Our framework predicts that any experimental system showing the latter would be evidence \emph{against} the Trinity hypothesis --- an important falsification criterion absent from previous theoretical work. This predicts an asymmetric fossil and synthetic-biology signature that extends the ``minimum life'' criteria developed in the autopoiesis literature \citep{ruizmirazo2004basic,luisi2003autopoiesis} with a specific ordering constraint absent from prior work.
...
\paragraph{The timescale mapping to physical systems is provisional.} A ``tick'' in our simulation does not correspond to a fixed physical duration without anchoring assumptions. A natural anchor is lipid lateral diffusion in fluid bilayers (typical $D_{\mathrm{lipid}} \approx 1$--$10\,\mu\mathrm{m}^2/\mathrm{s}$ for oleic-acid and POPC fluid phase; \citealt{vaz1984lateral,machan2010lipid}) on a $\sim 1.5\,\mu\mathrm{m}$ vesicle, yielding $1\text{ tick} \approx 0.45\,\mathrm{s}$ at $D = 5\,\mu\mathrm{m}^2/\mathrm{s}$. Under this provisional anchor the headline 1D Clock-to-Map delay (243 ticks) corresponds to $\approx 1.8$ minutes and the 2D delay (56 ticks) to $\approx 25$ seconds --- both squarely within the time-resolution window of standard fluorescence microscopy of Rh-DHPE-doped vesicles. We emphasize that this calibration is illustrative, not predictive: the precise tick-to-second ratio depends on lipid composition, temperature, and the specific reaction-diffusion coefficients of the prebiotic chemistry, all of which would need to be measured in any wet-lab translation. What it establishes is that the predicted delay is observable, not infinitesimal or geological.

\subsection{Experimental Predictions}

We propose the following testable predictions for laboratory validation:

\paragraph{Prediction 1: Ordering in vesicle populations.} In a population of fatty acid vesicles growing under micelle feeding conditions, lineages that develop spatial organization on their membranes (measurable via fluorescent lipid probes \citep{dealmeida2003sphingomyelin}, pH indicators, or protein localization assays \citep{kurihara2011self}) should first exhibit division-interval regularization. The prediction is that no lineage should develop stable spatial patterning \emph{before} its division CV drops below threshold.
...
\paragraph{Prediction 4: A quantitative CV threshold.} In our simulations, Phase B (Clock-locked) is defined by population-mean $\mathrm{CV} < 0.25$, and populations that reached Phase C consistently exhibited per-cell division CV of $0.045 \pm 0.023$ --- well below the 0.25 Phase B threshold and tightly matching the independent Vesicle Division study ($\mathrm{CV} = 0.06$, \citealt{farina2025vesicle}). We therefore predict that vesicles exhibiting division CV below approximately 0.15 should be capable of developing persistent spatial organization, while those with CV above 0.3 should not. The precise threshold in physical systems will depend on system-specific timescales and chemical-noise levels and should be refined experimentally, but the qualitative prediction --- a sharp transition in patterning capability as division regularity crosses a system-specific threshold --- is falsifiable with present-day vesicle microscopy. Experimentally, division CV in bacterial populations typically falls in the range $0.15$--$0.25$ \citep{stewart2005aging,taheriaraghi2015cellsize}, consistent with our simulated threshold and providing a natural reference scale for vesicle experiments. A complementary ``speed-limit'' prediction follows from the Damk\"ohler analysis of \S5.3: at sufficiently high lipid supply, vesicles whose division CV satisfies the regularity criterion may still fail to develop persistent spatial organization because the cell-cycle period drops below the pattern-consolidation time. The falsifiable boundary is therefore two-sided --- the Map fails both above CV $\approx 0.3$ (too irregular) and below approximately $T_{\mathrm{div}} \approx \tau_{\mathrm{pattern}}$ (too fast). Wet-lab tests can sweep lipid supply rate to locate the lower edge.

% ════════════════════════════════════════════════════════════════════
\section{Conclusion}

Biological agency emerges through a mandatory sequence: temporal regularity (Clock) precedes spatial organization (Map), which precedes thermodynamic selection (Engine). Across 1{,}900 independent simulations --- with 1{,}845 reaching both of the critical transitions --- spanning two geometries and twelve parameter conditions, we observed zero violations of this ordering. The principle is robust, geometry-independent, parameter-independent, and does not rely on specific Turing bifurcation regimes. It appears to reflect a general physical constraint on coupled temporal-spatial systems: persistent spatial structure requires temporal continuity to accumulate, and mechanisms that destroy temporal continuity (irregular division, in our case) prevent spatial organization from consolidating. We offer this as a testable proposal rather than a settled principle.
...
Our results suggest that the transition from matter to life may be understood not as a singularity requiring extraordinary explanation, but as a phase transition governed by a specific ordering principle --- one we can articulate, have now simulated at scale, and propose specific laboratory tests for. Whether this ordering is universal across all possible realizations of biological agency, or specific to the class of coupled dissipative systems modeled here, is a question our simulations motivate but do not yet settle.
